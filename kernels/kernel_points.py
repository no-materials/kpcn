...
def kernel_point_optimization_debug(radius, num_points, num_kernels=1, dimension=3, fixed='center', ratio=1.0,
                                    verbose=0):
    """
    ...
    # Factor multiplying gradients for moving points (~learning rate)
    moving_factor = 1e-2
    ...
    # Optional fixing ('center' has first kernel point as (0, 0, 0) for all kernels)
    if fixed == 'center':
        kernel_points[:, 0, :] *= 0
    if fixed == 'verticals':
        kernel_points[:, :3, :] *= 0
    ...
    for iter in range(10000):
        ...
        saved_gradient_norms[iter, :] = np.max(gradients_norms, axis=1)  # for each kernel,store max norm of all points
